...
### 2. 自动化工具箱与一键部署 (Tools & Installation)

YuLink 提供了全自动化的批处理脚本矩阵，零门槛开箱即用：

| 脚本工具 | 功能说明 | 使用场景 |
| :--- | :--- | :--- |
| **`一键安装.bat`** 🚀 | **一键全自动部署**：自动诊断依赖、按需静默补齐、脱机编译、写入证书信任与注册 Office | **首次安装 / 升级更新** |
| **`环境检查与修复.bat`** 🔍 | **运行环境一键检测与自动修复**：自动扫描 .NET 4.8 与 WebView2，缺失时自动从微软官方拉取并安装 | **环境排查 / 系统精简版补齐** |
| **`一键卸载.bat`** 🗑️ | **一键完全卸载**：清理所有注册表加载项、COM 项与 ClickOnce 信任残留 | **彻底移除插件** |

#### 🚀 快速上手：
1. 下载本项目发布包或克隆代码仓库到本地。
2. 双击运行 **`一键安装.bat`**（会自动完成环境自愈与插件注册）。
